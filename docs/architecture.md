...
Kalender tidak membuka tumpukan rute baru ketika tanggal dipilih; detail transaksi dan form pencatatan tetap memakai rute ledger yang sudah ada. Daftar Anggaran ditanam sebagai isi tab tanpa `Scaffold` atau FAB bersarang dan dibaca dari provider anggarannya sendiri, sehingga error/loading `financeSnapshotProvider` tidak memblokirnya. Daftar tersebut memakai navigator bulan untuk Semua/Bulanan/Kustom dan navigator tahun untuk Tahunan, bukan tab status Aktif/Mendatang/Riwayat; bulan/tahun berjalan menjadi batas kanan. Anggaran tidak diduplikasi sebagai kartu pada Ikhtisar karena sudah mempunyai tujuan navigasi utama. Rute `/budgets` tetap tersedia sebagai layar mandiri/deep link, sementara tambah/detail/edit didorong di atas layar asal. Ringkasan Bulanan juga bukan tujuan navigasi bawah: area **Arus bulan dipilih** mendorong rute `/summary?month=YYYY-MM`, sehingga bulan Ikhtisar menjadi konteks awal yang ditandai pada layar detail. Backup/restore tetap merupakan alur pemeliharaan data dari menu aplikasi. Tujuan Keuangan tidak otomatis menjadi tujuan keenam; pintu awalnya direncanakan melalui Ikhtisar dan menu.
...
Total header pemasukan/pengeluaran diturunkan dari jumlah allocation dan disimpan agar perhitungan saldo tetap sederhana. Saldo, arus kas total, jumlah transaksi, riwayat, dan kalender menjumlahkan header tepat sekali. Diagram lingkaran arus bulan pada kartu Saldo utama memakai agregat header `FinanceSnapshot.income` dan `FinanceSnapshot.expense` untuk bulan yang dipilih; transfer serta penyesuaian tidak ikut dihitung. Anggaran, filter kategori, dan diagram kategori menjumlahkan nominal allocation. Karena itu transaksi Rp17.000 dengan Rp15.000 Makan serta Rp2.000 Parkir tetap satu transaksi dan satu pengurangan saldo, tetapi berkontribusi tepat ke dua kategori.
...
Anggaran v1 mendukung CRUD untuk periode bulanan, tahunan kalender, dan rentang tanggal kustom dengan satu atau beberapa ID subkategori pengeluaran. `BudgetBrowseFilter` memilih budget yang beririsan dengan jendela bulan/tahun dan memberi `usageThroughDay` sebagai cutoff agregasi historis; kelompok hasil memakai kombinasi jenis serta rentang agar total bulanan, tahunan, dan kustom tidak tercampur. Progres dihitung dari `ledger_allocations.amount`, sedangkan constraint/repository menolak kategori yang sama pada rentang inklusif yang beririsan. `copyMonthlyBudgets` menyalin definisi bulan sebelumnya dalam satu transaksi setelah validasi ulang dan menghasilkan row independen tanpa hubungan sinkronisasi. Seluruhnya memakai tabel schema v6 dan payload backup v4 yang sudah ada. Kontrak lengkap dijelaskan dalam [spesifikasi Anggaran v1](budgets.md).

[Tujuan Keuangan v1](financial-goals.md) dirancang sebagai overlay alokasi current-state pada rekening nyata, bukan jenis ledger, rekening virtual, atau pengurang saldo. Kenaikan alokasi nantinya dibatasi saldo yang belum dialokasikan saat operasi berlangsung, sedangkan mutasi ledger berikutnya tetap diperbolehkan dan dapat menghasilkan peringatan saldo sumber kurang. Rencana ini memakai tabel `financial_goals` serta `goal_allocations` pada schema v7 dan payload backup v5, tetapi keduanya belum aktif.
...
Bagian **Tren terkini** pada Ikhtisar berdiri sendiri dari pemilih bulan. Diagram batang mengelompokkan header pengeluaran berdasarkan `occurredDay` untuk hari ini dan enam hari sebelumnya; tanggal tanpa pengeluaran tetap menghasilkan titik nol. Nominal header dijumlahkan tepat sekali agar transaksi split tidak dihitung ganda. Pemasukan, transfer, dan penyesuaian tidak masuk grafik ini.

Diagram garis mempunyai tujuh titik: saldo pada akhir masing-masing dari enam bulan yang sudah selesai, lalu Saldo utama saat ini. Saldo historis dihitung ulang dari seluruh ledger sampai tanggal titik—pemasukan menambah, pengeluaran mengurangi, transfer mengurangi sumber dan menambah tujuan, sedangkan penyesuaian memakai nominal bertanda.

Riwayat perubahan `balanceGroup` belum disimpan. Karena itu, kelompok rekening **saat ini** diterapkan secara retroaktif ketika titik historis dihitung. Rekening yang saat ini berkelompok Saldo utama tetap disertakan pada titik lampau walaupun kini diarsipkan, agar jejak saldo sebelum arsip tidak hilang. Titik saat ini tetap mengikuti subtotal Saldo utama normal: hanya rekening aktif dalam kelompok tersebut; invariant arsip bersaldo nol menjaga kedua perhitungan konsisten.
...
Kalender dan split transaction memakai `ledger_allocations` sejak schema v4/payload v2. Kelompok Saldo utama serta Simpanan & investasi memakai schema v5/payload v3 dengan `balanceGroup`. Anggaran kini aktif pada schema v6/payload v4; restore v1–v3 tetap didukung dengan anggaran kosong, sementara v1/v2 memetakan rekening lama ke Saldo utama. Smoke test perangkat fisik untuk alur Anggaran dan backup–restore v4 berhasil pada 9 September 2026. Tujuan keuangan baru mempunyai kontrak rancangan dan belum termasuk fitur aktif. Diagram lingkaran arus bulan, pengeluaran tujuh hari, dan tren Saldo utama ringkas sudah tersedia pada Ikhtisar; komposisi kategori, analitik anggaran/tujuan, dan fitur Diagram penuh serta utang/piutang belum tersedia. Lihat [product brief](product-brief.md) untuk urutan ruang lingkup produk.
