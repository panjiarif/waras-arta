...
void main() {
  ...
  testWidgets('more menu opens encrypted backup screen on a narrow phone', (
    ...
    await tester.pumpAndSettle();
    expect(find.byKey(const Key('manage-budgets-menu')), findsNothing);
    expect(find.text('Kelola anggaran'), findsNothing);
  ...
  testWidgets('overview keeps summaries in their intended reading order', (
    ...
    expect(
      find.textContaining('tidak termasuk simpanan & investasi'),
      findsNothing,
    );
  ...
  testWidgets('budget action replaces the transaction action immediately', (
    tester,
  ) async {
    await pumpApp(tester, _UiRepository(withAccounts: true));
    ...
    expect(find.byKey(const Key('budget-name')), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
  ...
  testWidgets(
    'calendar day totals stay side by side across a realistic phone width',
    (tester) async {
      ...
      repository.entries.addAll([
        ...
        _financeEntry(
          ...
          accountId: 1,
          amount: 2750000,
          categoryName: 'Kebutuhan bulanan',
          categoryIconKey: 'shopping_bag',
          note: '',
          occurredAt: date,
          createdAt: date,
      ...
      final totalsRow = find.byKey(const Key('calendar-day-totals-row'));
      final income = find.byKey(const ValueKey('calendar-day-metric-income'));
      final expense = find.byKey(const ValueKey('calendar-day-metric-expense'));
    ...
    (tester) async {
      ...
      expect(
        find.descendant(
          ...
          matching: find.bySemanticsLabel(RegExp('Anggaran')),
        ),
        findsOneWidget,
      );
      await tester.tap(find.byKey(const Key('budget-tab')));
  ...
  testWidgets(
    'budget tab keeps filters when switching tabs and opening detail',
    (tester) async {
      ...
      await tester.scrollUntilVisible(
        find.byKey(const ValueKey('budget-card-14')),
        180,
        scrollable: budgetListScrollable,
      );
      final offsetBeforeSwitch = tester
          .state<ScrollableState>(budgetListScrollable)
          .position
          .pixels;
  ...
  testWidgets(
    'standalone budget route resets filters and keeps its back stack',
    (tester) async {
      final budgets = FakeBudgetRepository(
        items: [
          fakeBudgetProgress(id: 1),
      ...
      unawaited(GoRouter.of(homeContext).push<void>('/budgets'));
      await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.pumpAndSettle();
      expect(openCreatedBudget, findsOneWidget);
      await tester.tap(openCreatedBudget);
      await tester.pumpAndSettle();
